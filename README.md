...
- **서버**: 가상 컴퓨터인 EC2에 퍼블릭 IP를 할당하고, 웹 서버 프로그램인 Nginx를 실행합니다.
- **보안**: Security Group으로 웹 접속은 허용하고 SSH 접속은 운영자 IP로 제한합니다.
- **권한**: IAM 사용자에게 EC2, VPC, Security Group을 구성하는 데 필요한 권한만 부여하고, 전체 관리자 권한은 부여하지 않습니다.
...
- **클라우드**: 인터넷으로 필요한 컴퓨터와 네트워크를 빌려 쓰는 환경
- **네트워크**: 여러 컴퓨터가 데이터를 주고받는 연결 구조
- **VPC**: AWS 안에 만드는 나만의 네트워크
- **Public Subnet**: 인터넷과 연결할 수 있는 VPC 내부 공간
- **서버(EC2)**: 웹 서비스를 실행하는 가상 컴퓨터
- **Nginx**: 웹 요청을 받아 페이지를 보내주는 프로그램
