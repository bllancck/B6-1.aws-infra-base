...
1. **작업 권한을 준비합니다.** 루트 계정 대신 실습에 필요한 권한만 가진 IAM 사용자를 사용합니다.
2. **AWS 안에 네트워크를 만듭니다.** VPC를 생성하고 그 안에 Public Subnet을 구성합니다.
3. **네트워크를 인터넷과 연결합니다.** Internet Gateway를 VPC에 연결하고 Route Table에 인터넷 경로를 추가합니다.
4. **접근 범위를 제한합니다.** Security Group에서 HTTP 80번 포트는 공개하고 SSH 22번 포트는 운영자 IP에만 허용합니다.
5. **웹 서버를 배포합니다.** Public Subnet에 EC2를 생성하고 `user-data`로 Nginx를 자동 설치합니다.
6. **서버 내부를 확인합니다.** SSH로 접속해 Nginx 실행 상태와 `localhost` 응답을 검사합니다.
7. **외부 접속을 확인합니다.** EC2의 Public IP로 웹 페이지와 `/health` 응답이 정상인지 검증합니다.
8. **문제와 해결 과정을 기록합니다.** 오류가 발생하면 증상, 가설, 검증, 조치, 결과, 재발 방지를 정리합니다.
9. **실습 리소스를 정리합니다.** 과금 방지를 위해 생성한 리소스를 삭제하고 체크리스트로 남은 항목이 없는지 확인합니다.
