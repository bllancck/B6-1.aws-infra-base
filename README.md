...
이 프로젝트는 AWS에서 외부 접속이 가능한 웹 서비스의 기초 인프라를 직접 설계·구축하며, **네트워크 흐름, 접근 제어, 최소 권한, 장애 진단, 리소스 정리**를 익히기 위한 실습입니다. 단순히 EC2 인스턴스를 실행하는 데서 끝나지 않고, 외부 HTTP 요청이 VPC 경계를 지나 Nginx까지 도달하려면 어떤 구성 요소가 필요한지 설명하고 검증하는 것을 목표로 합니다.

서울 리전(`ap-northeast-2`)에 격리된 VPC와 퍼블릭 서브넷을 만들고, Internet Gateway와 Route Table로 인터넷 경로를 구성했습니다. 퍼블릭 서브넷의 EC2에는 Nginx를 배포했으며, Security Group은 HTTP(80)는 전체에 공개하되 SSH(22)는 운영자 IP로 제한했습니다. AWS 리소스를 다루는 IAM 사용자에게는 EC2·VPC 구성에 필요한 권한만 부여해 최소 권한 원칙을 적용했습니다.

인프라 생성, 요구사항 검증, 리소스 삭제 과정은 AWS CLI 기반 스크립트로 자동화했습니다. 저장소에는 실행 코드뿐 아니라 아키텍처 다이어그램, 외부 접속 검증 기록, 실제 장애의 원인 분석과 해결 과정, 과금 방지를 위한 정리 체크리스트도 함께 담겨 있습니다.

이 구성은 학습을 위한 단일 AZ·단일 EC2 환경으로, 고가용성을 고려한 운영용 아키텍처는 아닙니다. 실습에 사용한 AWS 리소스는 과금 방지를 위해 모두 삭제했으며, README의 퍼블릭 IP와 접속 결과는 구축 당시의 검증 기록입니다.
