...
| 핵심 요소 | 간단한 뜻 | 이 프로젝트에서 |
|-----------|-----------|-----------------|
| **클라우드** | 인터넷으로 필요한 컴퓨터와 네트워크를 빌려 쓰는 환경 | AWS에 네트워크와 가상 서버를 생성해 사용합니다. |
| **네트워크** | 여러 컴퓨터가 데이터를 주고받는 연결 구조 | Internet Gateway와 Route Table로 VPC를 인터넷에 연결합니다. |
| **VPC** | AWS 안에 만드는 격리된 나만의 네트워크 | `10.0.0.0/16` 범위의 네트워크를 구성합니다. |
| **Public Subnet** | 인터넷과 통신할 수 있도록 설정한 VPC 내부 공간 | `10.0.1.0/24` 범위에 EC2를 배치하고 퍼블릭 IP를 할당합니다. |
| **서버(EC2)** | 웹 서비스를 실행하는 가상 컴퓨터 | Ubuntu EC2에서 Nginx를 실행합니다. |
| **Nginx** | 웹 요청을 받아 응답을 보내는 웹 서버 프로그램 | HTTP 80번 포트에서 웹 페이지와 `/health` 응답을 제공합니다. |
| **보안(Security Group)** | 서버로 들어오는 네트워크 접근을 제어하는 규칙 | HTTP 80은 공개하고 SSH 22는 운영자 IP에만 허용합니다. |
| **권한(IAM)** | AWS 리소스를 다룰 수 있는 작업 범위 | EC2, VPC, Security Group에 필요한 권한만 부여하고 관리자 권한은 제외합니다. |
