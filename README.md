...
| 영역 | 주요 구성 요소 | 적용 내용 |
|------|----------------|-----------|
| **네트워크** | VPC(격리 네트워크), Public Subnet(인터넷 연결 구역), Internet Gateway(인터넷 출입구), Route Table(통신 경로) | VPC 내부에 Public Subnet을 만들고 인터넷 통신 경로를 구성합니다. |
| **서버** | EC2(가상 컴퓨터), Public IP(외부 접속 주소), Nginx(웹 서버 프로그램) | EC2에 Nginx를 배포하고 외부에서 접속할 수 있도록 Public IP를 할당합니다. |
| **보안** | Security Group(가상 방화벽), HTTP 80, SSH 22 | 웹 접속은 공개하고 서버 관리 접속은 운영자 IP로 제한합니다. |
| **권한** | IAM 사용자·정책(AWS 작업 권한), 최소 권한 원칙 | 인프라 구성에 필요한 권한만 허용하고 전체 관리자 권한은 제외합니다. |
