...
서울 리전에 격리된 VPC 를 직접 설계하고, 퍼블릭 서브넷의 EC2 에 Nginx 를 배포해 외부에서 접속 가능한 웹 서비스를 구성한 기록입니다.
보안 그룹과 IAM 에 최소 권한 원칙을 적용했고, 인프라 생성·검증·정리를 AWS CLI 스크립트로 재현 가능하게 만들었습니다.
